...
配置web.xml的类需要实现接口`AbstractAnnotationConfigDispatcherServletInitializer`，并实现三个关键的方法，Spring会自动查找该实现类来代替web.xml
...
1. JSON数据
   ...
   Json数据内容由HashMap转化而来，Json转化驱动可以是`jackjson`、`fastjson`，需要添加maven依赖。
